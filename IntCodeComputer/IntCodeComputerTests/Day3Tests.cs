...
namespace IntCodeComputerTests {
    ...
    public class Day3Tests {
        ...
        public void MultiplyBigNumbers() {
            Computer computer = new Computer(new long[] { 1102, 34463338, 34463338, 3, 99 });
            computer.Calculate();

            Assert.AreEqual(computer.Memory[3], (double)34463338 * (double)34463338);
        }

        [TestMethod]
        public void PrintBigNumber() {
            Computer computer = new Computer(new long[] { 1102, 34915192, 34915192, 7, 4, 7, 99, 0 });
            var output = computer.Calculate();

            Assert.IsTrue(output.output >= 1000000000000000);
            Assert.IsTrue(output.output < 10000000000000000);
        }

        [TestMethod]
        public void OutputMiddleNumber() {
            Computer computer = new Computer(new long[] { 104, 1125899906842624, 99 });
            var output = computer.Calculate();

            Assert.AreEqual(output.output, 1125899906842624);
        }

        [TestMethod]
        public void OutputPuzzleAns() {
            ...
            Assert.AreEqual(output.output, 3454977209);

            computer = new Computer(input);
            output = computer.Calculate(new List<int>() { 2 });

            Assert.AreEqual(output.output, 50120);
        }
    }
}
